...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _doubleAnimationSignUpW = Tween<double>(begin: widd(80), end: widd(90))
        .animate(
            CurvedAnimation(parent: _controllerLogin, curve: Curves.linear));
    _doubleAnimationSignUpH = Tween<double>(begin: hidd(54), end: hidd(54))
        .animate(
            CurvedAnimation(parent: _controllerLogin, curve: Curves.linear));
    _doubleAnimationSignUpR = Tween<double>(begin: 5, end: 10).animate(
        CurvedAnimation(parent: _controllerLogin, curve: Curves.linear));

    _doubleAnimationLoginW = Tween<double>(begin: widd(90), end: widd(80))
        .animate(
            CurvedAnimation(parent: _controllerLogin, curve: Curves.linear));
    _doubleAnimationLoginH = Tween<double>(begin: hidd(46), end: hidd(41))
        .animate(
            CurvedAnimation(parent: _controllerLogin, curve: Curves.linear));
  ...
  goEnd() async {
    ...
    if (state.type == LogSign.signUp) {
      ...
      _controllerBall1
          .forward()
          .then((value) => Go.replaceSlideAnim(context, ProviderLogin()));
    } else {
  ...
  Widget build(BuildContext context) {
    final state = Provider.of<MyState>(context);
    if (state.pageValue == 1) goEnd();
    return Container(
      decoration: BoxDecoration(
          image: DecorationImage(
              fit: BoxFit.fill, image: AssetImage('assets/images/bcsm.png'))),
      child: IndexedStack(
        index: state.pageValue,
